# -*- coding: utf-8 -*-
"""
Created on Tue Jul 19 14:38:09 2022
...
if __name__ == "__main__":
    ...
    env_complex = VirtualComplexMachineReachEnv(distance_threshold = 0.03,
    ...
    # -- create tranfer environment 
    factor = np.array([5.5, 6])
    transf_matrix = np.array([[factor[0], 0, 0], [0, factor[1], 0], [0, 0, 1]])
   
    env_stages = VirtualAxesIntoBoxEnv(axes_range = np.array([[-50, 50], [-25, 25]]), 
                          max_episode_steps = 5,
    ...
    transfer_stages_path = '../training_axes/experiments/models/intobox_new'

    policy_library = []
    
    policy_library.append(SAC.load(transfer_scan_path, env=env_scan).policy)
    policy_library.append(SAC.load(transfer_stages_path, env=env_stages).policy)
        
    #print(f'policy library is {policy_library}')
    train(env_complex, result_file, None,  policy_library)
    #train(env_complex, result_file)
